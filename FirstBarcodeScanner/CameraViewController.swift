...
class CameraViewController: UIViewController, AVCaptureMetadataOutputObjectsDelegate, ItemSelectionViewControllerDelegate {
    ...
    private func createMetadataObjectOverlayWithMetadataObject(_ metadataObject: AVMetadataObject) -> MetadataObjectLayer {
        ...
        if let barcodeMetadataObject = transformedMetadataObject as? AVMetadataMachineReadableCodeObject {
            ...
            if let textLayerString = textLayerString {
                ...
                self.sessionQueue.async {
                    self.recognizedBarcode = textLayerString
                    self.hasFoundBarcode = true
                }
    ...
    private var hasFoundBarcode:Bool = false
    private var isCheckingBarcode:Bool = false
    
    private func getProductDetails() {
        DispatchQueue.main.async {
            if self.hasFoundBarcode == true && self.isCheckingBarcode == false {
                ...
                if let url = URL(string: "https://api.walmartlabs.com/v1/items?apiKey=dpzkrusjdxa2hq54wj5wyked&upc="+self.recognizedBarcode.dropFirst()) {
                    let session = URLSession.shared
                    
                    let task = session.dataTask(with: url) {
                        (data, response, error) in
                        if error != nil {
                            let alertMsg = "Unable to connect server"
                            let message = NSLocalizedString(alertMsg, comment: "Alert message when something goes wrong during server connection")
                            let alertController = UIAlertController(title: "AVCamBarcode", message: message, preferredStyle: .alert)
                            alertController.addAction(UIAlertAction(title: NSLocalizedString("OK", comment: "Alert OK button"), style: .cancel, handler: nil))
                            self.present(alertController, animated: true, completion: nil)
                        } else {
                            if let urlContent = data {
                                do {
                                    ...
                                    if jsonResult.object(forKey:"items") != nil {
                                        let name = ((jsonResult["items"] as! Array)[0] as AnyObject)["name"] as! String
                                        var desc = ((jsonResult["items"] as! Array)[0] as AnyObject)["shortDescription"] as? String
                                        if desc == nil {
                                            desc = ((jsonResult["items"] as! Array)[0] as AnyObject)["longDescription"] as? String
                                        }
                                        let imageUrl = ((((jsonResult["items"] as! Array)[0] as AnyObject)["imageEntities"] as! Array)[0] as AnyObject)["mediumImage"] as! String
                                        let productUrl = ((jsonResult["items"] as! Array)[0] as AnyObject)["productUrl"] as! String
                                        let salePrice = "USD$"+(((((jsonResult["items"] as! Array)[0] as AnyObject)["salePrice"]) as! NSNumber).stringValue)
                                        let avail = ((jsonResult["items"] as! Array)[0] as AnyObject)["stock"] as! String
                                        let upc = ((jsonResult["items"] as! Array)[0] as AnyObject)["upc"] as! String
                                        self.showProductDetails(name, desc!, imageUrl, productUrl, salePrice, avail, upc)
                                    }
                                    if jsonResult.object(forKey:"errors") != nil {
                                        let alertMsg = "Product Not Found"
                                        let message = NSLocalizedString(alertMsg, comment: "Alert message when something goes wrong during product search")
                                        let alertController = UIAlertController(title: "AVCamBarcode", message: message, preferredStyle: .alert)
                                        alertController.addAction(UIAlertAction(title: NSLocalizedString("OK", comment: "Alert OK button"), style: .cancel, handler: nil))
                                        
                                        self.present(alertController, animated: true, completion: nil)
                                        self.doneProductView()
                                    }
                                } catch {
                                    let alertMsg = "Server error"
                                    let message = NSLocalizedString(alertMsg, comment: "Alert message when something goes wrong during product search")
                                    let alertController = UIAlertController(title: "AVCamBarcode", message: message, preferredStyle: .alert)
                                    alertController.addAction(UIAlertAction(title: NSLocalizedString("OK", comment: "Alert OK button"), style: .cancel, handler: nil))
    ...
    private func showProductDetails(_ name: String, _ desc: String, _ imageUrl: String, _ productUrl: String, _ salePrice: String, _ avail: String, _ upc: String) {
        let allSessionPresets: [AVMetadataObject.ObjectType] = [AVMetadataObject.ObjectType(rawValue: "Barcode: "+upc), AVMetadataObject.ObjectType(rawValue: "Name: "+name), AVMetadataObject.ObjectType(rawValue: "Description: "+desc), AVMetadataObject.ObjectType(rawValue: "Price: "+salePrice), AVMetadataObject.ObjectType(rawValue: "Availability: "+avail), AVMetadataObject.ObjectType(rawValue: "Data Source: WalMart Open API")]
        
        let itemSelectionViewController = ItemSelectionViewController<AVMetadataObject.ObjectType>(delegate: self,
                                                                                                   identifier: productItemSelectionIdentifier,
                                                                                                   allItems: allSessionPresets,
                                                                                                   selectedItems: [],
                                                                                                   allowsMultipleSelection: false, allowsSelection: false)
    ...
    let productItemSelectionIdentifier = "ProductDetails"
    ...
    func itemSelectionViewController<Item>(_ itemSelectionViewController: ItemSelectionViewController<Item>, didFinishSelectingItems selectedItems: [Item]) {
        ...
        } else if identifier == productItemSelectionIdentifier {
            self.doneProductView()
        }
    }
    
    func doneProductView() {
        DispatchQueue.main.async {
            self.session.startRunning()
            self.isSessionRunning = self.session.isRunning
            self.metadataObjectTypesButton.isEnabled = true
            ...
            self.cameraButton.isEnabled = true
            self.zoomSlider.isEnabled = true
            self.recognizedBarcode = ""
            self.isCheckingBarcode = false
            self.hasFoundBarcode = false
        }
    }
